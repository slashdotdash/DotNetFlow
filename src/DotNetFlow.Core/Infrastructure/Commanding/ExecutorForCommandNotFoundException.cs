...
namespace DotNetFlow.Core.Infrastructure.Commanding
{
    ...
    public class ExecutorForCommandNotFoundException : Exception
    {
        ...
        public ExecutorForCommandNotFoundException(Type commandType)
        ...
        /// <exception cref="ArgumentNullException">Occurs when <i>commandType</i> is a <c>null</c> dereference.</exception>
        public ExecutorForCommandNotFoundException(Type commandType, string message, Exception inner)
            : base((String.IsNullOrEmpty(message) ? String.Format("No handler was found for command {0}.", commandType.FullName) : message), inner)
        {
